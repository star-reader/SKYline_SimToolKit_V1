...
import mapboxgl, { type Map, type StyleSpecification, type FogSpecification } from "mapbox-gl"
...
const addSKYlineLayer_old = (map: Map) => {
    ...
    map.addSource('navdata',{
        'type': 'vector',
        'tiles': ['https://api.skylineflyleague.cn/efb/mapbox-enroute-beta/enroute/{z}/{x}/{y}'],
        'maxzoom': 7,
        'minzoom': 0,
        "format": "pbf",
    })
    ...
    map.addLayer({
        ...
        paint:{
            ...
            "fill-extrusion-pattern": "terminal",
...
const changeMapTheme = (theme: 'light' | 'dark', map: mapboxgl.Map) => {
    ...
    const isUnSateLayer = (preset !== 'World Satellite' && preset !== 'Route Map' && preset !== 'Route Map Low')
